...
```{r remove 10 and 27}
## find all cells that are labelled as such
names_of_genotype_fail_cells <- rownames(pheno[(pheno$sub_identity_updated %in% c("GCSKO-10", "WT-10", "GCSKO-27", "GCSKO-28")), ])
...
## check dimensions
paste("number of genes in counts", dim(counts)[1], "number of cells in counts", dim(counts)[2])
paste("number of columns in pheno", dim(pheno)[2], "number of cells in pheno", dim(pheno)[1])
```

### change md5 --> md3, md3 -->md4, md4 --> md5 

```{r}
## inspect original
table(pheno$identity_name_updated)
table(pheno$sub_name_updated)

## change md3 --> mdx
pheno$identity_name_updated[pheno$identity_name_updated == "md3"] <- "mdx"
pheno$sub_name_updated[pheno$sub_name_updated == "md3"] <- "mdx"
pheno$sub_name_updated[pheno$sub_name_updated == "WT-md3"] <- "WT-mdx"

## change md5 --> md3 
pheno$identity_name_updated[pheno$identity_name_updated == "md5"] <- "md3"
pheno$sub_name_updated[pheno$sub_name_updated == "md5"] <- "md3"
pheno$sub_name_updated[pheno$sub_name_updated == "WT-md5"] <- "WT-md3"

## change md4 --> md5
pheno$identity_name_updated[pheno$identity_name_updated == "md4"] <- "md5"
pheno$sub_name_updated[pheno$sub_name_updated == "md4"] <- "md5"
pheno$sub_name_updated[pheno$sub_name_updated == "WT-md4"] <- "WT-md5"

## change mdx --> md4
pheno$identity_name_updated[pheno$identity_name_updated == "mdx"] <- "md4"
pheno$sub_name_updated[pheno$sub_name_updated == "mdx"] <- "md4"
pheno$sub_name_updated[pheno$sub_name_updated == "WT-mdx"] <- "WT-md4"
...
v1 <- VlnPlot(object = ss2_mutants, features = "percent.mt", group.by = "sub_name_updated", pt.size = 0.01) + 
...
```{r}
df <- data.frame(nCount = log10(ss2_mutants@meta.data$nCount_RNA), nGenes = ss2_mutants@meta.data$nFeature_RNA, identity = ss2_mutants@meta.data$identity_name_updated, identity_gene = ss2_mutants@meta.data$identity_gene_updated, percent_mt = ss2_mutants@meta.data$percent.mt)
```
...
```{r}
## extract data
df_cells_recovered <- as.data.frame(table(ss2_mutants_final@meta.data$sub_name_updated))
df_cells_profiled <- as.data.frame(table(pheno$sub_name_updated))
...
```{r}
##plot
## can add ",fill=genotype" to first statement to get colours by genotype
QC_by_genotype <- ggplot(df_cells_recovered, aes(x=genotype, y=recovered_pc)) + 
  geom_bar(stat = "identity", fill = "grey") +
  ylim(0, 100) +
...
## Mutant 20 = fd4
## Mutant 2 = md3
QC_failed_plates <- grid.arrange(arrangeGrob(plot_list[[1]] + theme(legend.position="none") + labs(title = paste("fd4", "\n", "plate 1")) + theme(plot.title = element_text(hjust = 0.5)),
                               plot_list[[2]] + theme(legend.position="none") + labs(title = paste("fd4", "\n", "plate 2")) + theme(plot.title = element_text(hjust = 0.5)),
                               plot_list[[3]] + theme(legend.position="none") + labs(title = paste("fd4", "\n", "plate 3")) + theme(plot.title = element_text(hjust = 0.5)),
                               plot_list[[4]] + theme(legend.position="none") + labs(title = paste("fd4", "\n", "plate 4")) + theme(plot.title = element_text(hjust = 0.5)),
                               plot_list[[5]] + theme(legend.position="none") + labs(title = paste("md4", "\n", "plate 1")) + theme(plot.title = element_text(hjust = 0.5)),
                               plot_list[[6]] + theme(legend.position="none") + labs(title = paste("md4", "\n", "plate 2")) + theme(plot.title = element_text(hjust = 0.5)), nrow=2), 
...
## save
ggsave("../images_to_export/QC_failed_plates.png", plot = QC_failed_plates, device = "png", path = NULL, scale = 1, width = 27, height = 25, units = "cm", dpi = 300, limitsize = TRUE)
```
...
HoverLocator(plot = plot, information = FetchData(ss2_mutants_final, vars = c("nFeature_RNA", "ident", "identity_name_updated")))
```
...
FeaturePlot(ss2_mutants_final, features = c("PBANKA-0515000", "PBANKA-1319500", "PBANKA-1212600","PBANKA-0600600", "PBANKA-1315700", "PBANKA-0416100", "PBANKA-1437500", "PBANKA-0831000", "PBANKA-1102200"), coord.fixed = TRUE, order = TRUE)
```
...
dot_plot_df <- as.data.frame.matrix(table(df_meta_data$RNA_snn_res.1, df_meta_data$identity_name_updated))

dot_plot_df_pc <- (as.data.frame.matrix(prop.table(table(df_meta_data$RNA_snn_res.1, df_meta_data$identity_name_updated), margin = 2)) * 100)
...
#                 seed.use = 1234, 
#                 min.dist = 0.35, 
#                 reduction.name = "umap_cxb", 
#                 reduction.key='umapcxb_')
# 
# ## View
# DimPlot(test, reduction = "umap_cxb", label = TRUE, repel = TRUE, label.size = 5, pt.size = 0.5, group.by = "identity_name_updated") +
#   coord_fixed() +
#   theme_void() +
#   theme(legend.position = "bottom") +
...
cellxgene_table <- read.table("../data/Smartseq2/cellxgene.txt", header = TRUE, sep = "\t", stringsAsFactors = TRUE)
rownames(cellxgene_table) <- cellxgene_table$X
## subset cols of interest
cellxgene_table <- cellxgene_table[ ,c("umap1", "umap2")]
## rename cols:
names(cellxgene_table) <- c("cellxgene_umap1", "cellxgene_umap2")
## make into matrix so it can be saved into seurat:
cellxgene_table <- as.matrix(cellxgene_table)

## subset for cells only in final object
cellxgene_table <- cellxgene_table[rownames(cellxgene_table) %in% colnames(ss2_mutants_final), ]

## make subset seurat with only these cells
cellxgene_seurat <- subset(ss2_mutants_final, cells = rownames(cellxgene_table))

## store this optimsed UMAP in a custom dim slot
cellxgene_seurat[["cellxgene"]] <- CreateDimReducObject(embeddings = cellxgene_table, key = "cellxgene_", assay = DefaultAssay(cellxgene_seurat))

## check
DimPlot(cellxgene_seurat, label = TRUE, repel = FALSE, pt.size = 0.05, dims = c(1,2), reduction = "cellxgene") + coord_fixed()
```
make final plot

```{r}
## make list of genotypes
list_of_genotypes <- unique(cellxgene_seurat@meta.data$identity_name_updated)

## redorder for better plotting
#list_of_genotypes <- list_of_genotypes[c(1, 3, 2, 4, 11, 7, 8, 12, 5, 10, 6, 9)]

## make a blank list
list_plots_cellxgene <- vector(mode = "list", length = length(list_of_genotypes))

## for loop
for(i in seq_along(list_of_genotypes)){
    ## make a list of cells to highlight
  list_of_cells <- rownames(cellxgene_seurat@meta.data[which(cellxgene_seurat@meta.data$identity_name_updated == list_of_genotypes[i]), ])
  plot <- DimPlot(cellxgene_seurat, label = FALSE, pt.size = 0.1, cells.highlight = list_of_cells, dims = c(1,2), reduction = "cellxgene") + 
  coord_fixed() + 
  scale_color_manual(values=c("#E2E2E2", "#31206E")) + 
  theme_void() + 
  labs(title = paste(list_of_genotypes[i])) + 
  theme(plot.title = element_text(hjust = 0.5, family="Arial", size = 15, face = "italic"), legend.position = "none")
  ## add to the list
  list_plots_cellxgene[[i]] <- plot
}
```

add plot for all genotypes coloured
```{r}
list_plots_cellxgene[[12]] <- DimPlot(cellxgene_seurat, label = FALSE, pt.size = 0.1, group.by = "identity_name_updated", dims = c(1,2), reduction = "cellxgene") + 
  coord_fixed() + 
  theme_void() + 
  labs(title = "All genotypes") + 
  theme(plot.title = element_text(hjust = 0.5, family="Arial", size = 15, face = "plain"), legend.position = "bottom", legend.title = element_blank()) +
  guides(color=guide_legend(ncol=3, override.aes = list(size=3)))
```

```{r, fig.height = 29.7, fig.width = 21}
## plot
composite_cellxgene <- plot_grid(list_plots_cellxgene[[9]], 
                                      list_plots_cellxgene[[10]], 
                                      list_plots_cellxgene[[7]], 
                                      list_plots_cellxgene[[2]], 
                                      list_plots_cellxgene[[4]], 
                                      list_plots_cellxgene[[8]], 
                                      list_plots_cellxgene[[11]], 
                                      list_plots_cellxgene[[6]], 
                                      list_plots_cellxgene[[1]], 
                                      list_plots_cellxgene[[5]], 
                                      list_plots_cellxgene[[3]] + theme(plot.title = element_text(hjust = 0.5, family="Arial", size = 13, face = "plain")),
                                      list_plots_cellxgene[[12]] + theme(plot.title = element_text(hjust = 0.5, family="Arial", size = 13, face = "plain")),
                                      nrow = 3)
...
save
```{r}
ggsave("../images_to_export/cellxgene_figure.png", plot = composite_cellxgene, device = "png", path = NULL, scale = 1, width = 21, height = 29.7, units = "cm", dpi = 300, limitsize = TRUE)
```

Save h5ad
```{r}
## save final object
saveRDS(cellxgene_seurat, file = "../data_to_export/cellxgene_seurat.RDS") ## to read

#library(SeuratDisk)
#SaveH5Seurat(cellxgene_seurat, filename = "../images_to_export/cellxgene_seurat.h5Seurat")
#Convert("../images_to_export/cellxgene_seurat.h5Seurat", dest = "h5ad")
```
...
## Extra code
```{r}
## this was run, and then the resultant pheno was saved and re-read in at the beginning
# dim(pheno)

## add new column for mutant names
## make blank col
# pheno$identity_name_updated <- NA
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_0102400")] <- "md3"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_0413400")] <- "md5"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_0413400_820")] <- "md5"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_0713500")] <- NA
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_0716500")] <- "md4"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_0828000")] <- "gd1"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_0902300")] <- "fd2"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_1144800")] <- "fd5"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_1302700")] <- "md1"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_1418100")] <- "fd3"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_1435200")] <- "fd4"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_1447900")] <- "md2"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "PBANKA_1454800")] <- "fd1"
# pheno$identity_name_updated[which(pheno$identity_gene_updated == "WT")] <- "wild-type"

## add new column for mutant sub-names
## make blank col
# pheno$sub_name_updated <- pheno$sub_identity_updated
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-10_820")] <- "md5"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-13")] <- "fd2"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-17")] <- "fd3"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-19")] <- "md4"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-2")] <- "md3"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-20")] <- "fd4"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-21")] <- "fd1"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-29")] <- "md2"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-3")] <- "gd1"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "GCSKO-oom")] <- "md1"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-13")] <- "WT-fd2"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-17")] <- "WT-fd3"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-19")] <- "WT-md4"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-2")] <- "WT-md3"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-20")] <- "WT-fd4"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-3")] <- "WT-gd1"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-820")] <- "WT-820"
# pheno$sub_name_updated[which(pheno$sub_identity_updated == "WT-820_3_5")] <- "WT-820_3_5"
# 
# dim(pheno)
# 
# write.csv(pheno, file = "../data/Smartseq2/pheno_2020_2.csv", 
# row.names = TRUE)
```

## Publication Figure

[Cowplot](https://wilkelab.org/cowplot/articles/plot_grid.html) , [patchwork](https://cran.r-project.org/web/packages/patchwork/vignettes/patchwork.html), and [ggpubr](http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/81-ggplot2-easy-way-to-mix-multiple-graphs-on-the-same-page/) can all allow multiple plots to be plotted together. 
...
## cowplot method
right_figures <- plot_grid(QC_mito_violin, QC_mito_graph, labels = c('B', 'C'), label_size = 12, nrow=2)
F_Figures <- plot_grid(sample_map_no_reads, platemap_failed_qc, position_qc_fail, nrow=1, label_size = 12)

plot_grid(QC_composite_plot, right_figures, QC_by_genotype, mapping_rate_plot, F_Figures, labels = c('A', '', 'D', 'E', 'F'), label_size = 12, ncol = 2, nrow=3, rel_heights = c(2, 1, 1))
