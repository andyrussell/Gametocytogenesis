...
```{r}
##Import gtf file:
gtf <- read.table("/Users/Andy/Desktop/GCSKO/GCSKO_analysis_git/data/Pberghei.gtf", sep="\t", header = FALSE)
head(gtf)
```
*N.B.This was the .gtf file used for feature coutning the data

### Counts and Pheno
```{r}
## read in counts
counts <- read.delim("/Users/Andy/Desktop/GCSKO/GCSKO_analysis_git/data/counts_2020.csv", sep = ",", stringsAsFactors = FALSE, row.names = 1)

## read in pheno
pheno <- read.delim("/Users/Andy/Desktop/GCSKO/GCSKO_analysis_git/data/pheno_2020.csv", sep = ",", stringsAsFactors = FALSE)
...
paste(length(which(rowSums(ss2_mutants@assays$RNA@counts) == 0)), " genes are not detected in any cell")
```

```{r, echo = FALSE}
paste(length(colnames(counts)) - length(colnames(ss2_mutants@assays$RNA@counts)), " cells have zero counts")
```
...
## plot
sample_map_no_reads <- ggplot(data=table_platemap, aes(x=Column, y=Row)) +
...
## save
ggsave("~/images_to_export/platemap_no_count.png", plot = sample_map_no_reads, device = "png", path = NULL, scale = 1, width = 10, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
...
## save
ggsave("~/images_to_export/QC_mito_violin.png", plot = QC_mito_violin, device = "png", path = NULL, scale = 1, width = 15, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
```
...
## save
ggsave("~/images_to_export/QC_mito_graph.png", plot = QC_mito_graph, device = "png", path = NULL, scale = 1, width = 10, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
```
...
## save
ggsave("~/images_to_export/QC_composite_plot.png", plot = QC_composite_plot, device = "png", path = NULL, scale = 1, width = 20, height = 20, units = "cm", dpi = 300, limitsize = TRUE)
```
...
## save
ggsave("~/images_to_export/platemap_filtered.png", plot = sample_map, device = "png", path = NULL, scale = 1, width = 10, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
...
## save
ggsave("~/images_to_export/QC_failed_position.png", plot = position_qc_fail, device = "png", path = NULL, scale = 1, width = 10, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
...
## save
ggsave("~/images_to_export/QC_mapping_rate.eps", plot = mapping_rate_plot, device = "eps", path = NULL, scale = 1, width = 20, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
...
## save
ggsave("~/images_to_export/QC_by_genotype.png", plot = QC_by_genotype, device = "png", path = NULL, scale = 1, width = 15, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
...
## make df
  ...
  ## plot
  p <- ggplot(data=table_platemap, aes(x=Column, y=Row)) +
  #set up the platemap layout
      geom_point(data=expand.grid(seq(1, 12), seq(1, 8)), aes(x=Var1, y=Var2), color="grey90", fill="white", shape=21, size=6) +
  #Change the shape and colour of points for a variable
  ...
  # make the font size in the legend bigger
    theme(legend.text=element_text(size=10, colour = "black"), legend.title=element_text(size=10))
...
## make aggregate plot
# QC_failed_plates <- (plot_list[[1]] + theme(legend.position="none") +
...
## save
ggsave("~/images_to_export/QC_failed_plates.png", plot = QC_failed_plates, device = "png", path = NULL, scale = 1, width = 22, height = 17, units = "cm", dpi = 300, limitsize = TRUE)
```
