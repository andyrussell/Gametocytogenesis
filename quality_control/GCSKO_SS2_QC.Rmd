...
sample_map_no_reads <- ggplot(data=table_platemap, aes(x=Column, y=Row_rev)) +
  ...
  #Improve aes of guides
    guides(colour = guide_colorbar(barwidth = 10, barheight = 1, title="percentage of cells in this well with zero counts")) +
...
platemap_failed_qc <- ggplot(data=table_platemap, aes(x=Column, y=Row_rev)) +
  ...
  #add labels for the x axis
    scale_x_continuous(position = "top", breaks=seq(1, 12)) +
  #Improve aes of guides
    guides(colour = guide_colorbar(barwidth = 10, barheight = 1, title="percentage of cells in this well that failed QC")) +
  # display legend on bottom and make text bigger
    theme(legend.position="bottom", legend.text=element_text(size=10), legend.title = element_text(size = 10))
...
## save
ggsave("../images_to_export/platemap_failed_qc.png", plot = platemap_failed_qc, device = "png", path = NULL, scale = 1, width = 10, height = 10, units = "cm", dpi = 300, limitsize = TRUE)
```
...
## save final object
saveRDS(ss2_mutants_final, file = "../data_export/ss2_mutants_final.RDS") 
ss2_mutants_final <- readRDS("../data_export/ss2_mutants_final.RDS")
```
