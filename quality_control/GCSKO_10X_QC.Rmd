...
<div style="width:500px; height:400px">![](../data/10X/24284_cellranger_output_1.png)</div>
<div style="width:500px; height:200px">![](../data/10X/24284_cellranger_output_2.png)</div>
...
# reduce overplotting for performance:
df_barcodes_to_plot <- df_barcodes %>% group_by(round(log10(barcode_rank) * 1000), round(log10(lib_size) * 1000), cell)
df_barcodes_to_plot <- df_barcodes_to_plot %>% filter(row_number() == 1)

## make the plot

barcode_plot <- ggplot(df_barcodes_to_plot, aes(x=barcode_rank, y=lib_size, colour = cell, theme_size = 40)) +
...
### AP2-G over-expression timecourse
Can also do with the AP2-G over-expression timecourse data:
```{r}
