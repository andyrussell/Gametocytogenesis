...
### 1. Look at whether screen hits are in the DE lists
...
screen_hits_selected$`new gene ID` <- gsub("_", "-", screen_hits_selected$`new gene ID`)
screen_hits_names <- screen_hits_selected$`new gene ID`
screen_hits_names <- gsub("_", "-", screen_hits_names)
...
screen_hits_in_DE_genes <- list()

## for loop that finds screen hits in DE lists
for (i in 1:length(list_of_DE_gene_lists)){
  screen_hits_in_DE_genes[[i]] <- list_of_DE_gene_lists[[i]][list_of_DE_gene_lists[[i]]$Gene.ID %in% screen_hits_names, ]
  if (nrow(screen_hits_in_DE_genes[[i]]) > 0){
  screen_hits_in_DE_genes[[i]]$mutant <- names(list_of_DE_gene_lists)[i]
  }
}

## then bind these all together in one dataframe for ease of visualisation and view
df_screen_hits_that_are_DE <- data.frame(matrix(ncol=13))
names(df_screen_hits_that_are_DE) <- names(screen_hits_in_DE_genes[[2]])
for (i in 1:length(list_of_DE_gene_lists)){
  if (nrow(screen_hits_in_DE_genes[[i]]) == 0) next
  df_screen_hits_that_are_DE <- rbind(df_screen_hits_that_are_DE, screen_hits_in_DE_genes[[i]])
}
## remove the na row that is added
df_screen_hits_that_are_DE <- df_screen_hits_that_are_DE[!is.na(df_screen_hits_that_are_DE$Gene.ID), ]

## add in screen phenotype
df_screen_hits_that_are_DE <-merge(df_screen_hits_that_are_DE ,screen_hits_selected, by.x = "Gene.ID", by.y =  "new gene ID", all.x = TRUE, all.y = FALSE)

head(df_screen_hits_that_are_DE)

View(df_screen_hits_that_are_DE)
```
...
### 2. Look at genes that are common between the lists
...
mutant_male_cells <- rownames(tenx.mutant.integrated.sex@meta.data[tenx.mutant.integrated.sex@meta.data$assignment == "male" & tenx.mutant.integrated.sex@meta.data$identity_combined %ni% c("wild-type (10x)", "fd1", "fd2", "fd3", "fd4"), ])
mutant_female_cells <- rownames(tenx.mutant.integrated.sex@meta.data[tenx.mutant.integrated.sex@meta.data$assignment == "female" & tenx.mutant.integrated.sex@meta.data$identity_combined %ni% c("wild-type (10x)", "gd1", "md1", "md2", "md3", "md4", "md5"), ])

## subset males 
mutant_male_seurat <- subset(tenx.mutant.integrated.sex, cells = mutant_male_cells)

## subset females <- 
mutant_female_seurat <- subset(tenx.mutant.integrated.sex, cells = mutant_female_cells)
