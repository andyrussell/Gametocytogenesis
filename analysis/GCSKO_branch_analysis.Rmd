...
marks <- markers_subset_annotated[!duplicated(markers_subset_annotated[,c('gene')]),]
marks <- marks[order(marks$p_val_adj), ]
...
## plot
## cowplot method
marker_gene_plot_all <- plot_grid(marker_gene_plot_DNAJ, marker_gene_plot_EFTU, marker_gene_plot_132100, marker_gene_plot_BIP, marker_gene_plot_SAHH, marker_gene_plot_PNP, marker_gene_plot_MD1, nrow=2)
...
pt_plot_2 <- ggplot(df_seurat_bipot, aes(x = old_pt_values, y = `PBANKA-1437500`, colour = seurat_clusters)) + 
  geom_point() + 
  #scale_y_continuous(limit=c(0.00001,NA),oob=censor) +
  geom_smooth(method = "gam", formula = y ~s(x)) +
  theme_classic() +
...
```{r}
ggsave("../images_to_export/branch_p_ptplot_ap2g.png", plot = pt_plot_2, device = "png", path = NULL, scale = 1, width = 7, height = 9, units = "cm", dpi = 300, limitsize = TRUE)
```

```{r}
pt_plot_2 <- ggplot(df_seurat_bipot, aes(x = old_pt_values, y = `PBANKA-0706800`, colour = seurat_clusters)) + 
  geom_point() + 
  #scale_y_continuous(limit=c(0.00001,NA),oob=censor) +
  geom_smooth(method = "gam", formula = y ~s(x)) +
  theme_classic() +
  ylim(-1, 5) +
  stat_cor(method = "pearson", aes(color = seurat_clusters), label.x.npc = 'left', label.y.npc = 'top') +
  labs(x = "Pseudotime", y = "Expression level", title = "dna-j (PBANKA_0706800)") +
  scale_fill_discrete_qualitative(palette = "Set 3")+
  scale_colour_discrete_qualitative(palette = "Set 3") +
  theme(legend.position = "none", plot.title = element_text(face = "italic"))
