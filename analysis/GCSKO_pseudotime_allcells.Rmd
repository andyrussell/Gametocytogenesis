...
We have merged the two datasets together. Here, we will perform pseudotime analysis on the dataset
...
# 2. Pseudotime Analysis Using Monocle {.tabset}
...
## plot
plot_cells(monocle.object.all, color_cells_by = "pseudotime", label_cell_groups=FALSE, cell_size = 1, x = 2, y = 1) +
  coord_fixed() +
  theme_void() +
...
### Plot Modules
```{r}
## create a dataframe with clusters and modules
cell_group_df_all <- tibble::tibble(cell=row.names(colData(monocle.object.all)), cell_group=colData(monocle.object.all)$seurat_clusters)
