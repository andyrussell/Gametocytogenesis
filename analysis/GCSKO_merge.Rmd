...
levels_to_plot <- levels(tenx.mutant.integrated@meta.data$integrated_snn_res.0.5)

plot_level <- function(level, level_index) {
  list_of_cells <- tenx.mutant.integrated@meta.data %>%
    filter(integrated_snn_res.0.5 == level) %>%
    row.names()
  uamp_plot <- DimPlot(tenx.mutant.integrated, label = FALSE, repel = TRUE, pt.size = 0.1, cells.highlight = list_of_cells, dims = c(2, 1), reduction = "DIM_UMAP") +
    ...
    theme_void() +
    labs(title = paste("cluster", level_index)) +
    theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
  return(uamp_plot)
}

list_UMAPs_by_cluster <- purrr::imap(levels_to_plot, plot_level)
...
wrap_plots(list_UMAPs_by_cluster)
```

### Pick final resolution 

We will look in more detail at cells as they enter the sexual trajectory later. The PCA clustering will be more appropriate in this high-resolution view. In order to subset these cells, we will use the UMAP clustering. 
...
plot_marker = function(marker_name,marker){
  plot <- FeaturePlot(tenx.mutant.integrated, features = marker, coord.fixed = TRUE, min.cutoff = "q1", dims = c(2,1), reduction = "DIM_UMAP", pt.size = 1, order = TRUE) + 
  theme_void() + 
...
markers = list("PBANKA-1319500"="CCP2 (Female)","PBANKA-0416100"="MG1 (Male)","PBANKA-1437500"="AP2G (Commitment)","PBANKA-0831000"="MSP1 (Schizont)","PBANKA-1102200"="MSP8 (Asexual)","PBANKA-1101300"="SBP1 (Ring)","PBANKA-0722600"="Fam-b2 (Ring)","PBANKA-0711900"=paste("(HSP70; Reporter)","\n", "PBANKA_0711900"))




plots = purrr::imap(markers,plot_marker)
marker_gene_plot_all <- wrap_plots(plots,nrow=3)
...
wrap_plots(list_of_density_plots) + coord_fixed()
...
tenx.mutant.integrated@meta.data <- tenx.mutant.integrated@meta.data %>% mutate(cluster_colours_figure = case_when(
  seurat_clusters %in% male_clusters ~ "Male",
  seurat_clusters %in% female_clusters ~ "Female",
  seurat_clusters %in% asex_clusters ~ "Asexual",
  seurat_clusters %in% bipotential_clusters ~ "Bipotential",
  TRUE ~ NA_character_
))
...
labels = c( "gd1"= "(PBANKA_0828000)","fd1"= "(PBANKA_1454800)","fd2"= "PBANKA_0902300","fd3"="(PBANKA_1418100)","fd4"="(PBANKA_1435200)","md1"="(PBANKA_1302700)","md2"="(PBANKA_1447900)","md3"="(PBANKA_0413400)","md4"="(PBANKA_0102400)","md5"="(PBANKA_0716500)")




genotypes = names(labels)
names(genotypes) = genotypes

get_cells <- function(gt){
  return(rownames(dplyr::filter(tenx.mutant.integrated@meta.data, identity_name_updated == gt)))
}

cells_by_gt<- purrr::map(as.list(genotypes),get_cells)

make_plot <- function(cell_list, genotype_name){
  plot <- DimPlot(tenx.mutant.integrated, label = FALSE, repel = TRUE, pt.size = 0.1, cells.highlight = cell_list, group.by = "exclude_for_sex_ratio", dims = c(2,1), reduction = "DIM_UMAP") + 
  coord_fixed() + 
  scale_color_manual(values=c("#E2E2E2", "#31206E")) + 
  theme_void() + 
  labs(title = paste(genotype_name, "\n", labels[genotype_name])) + 
  ...
  annotate("text", x = 2, y = 2.8, label = female_symbol, size=7, color="gray")+ theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
  
  return(plot)
} 


plots<- purrr::imap(cells_by_gt,make_plot)
...
## plot composite plot
mutant_cell_locations <- wrap_plots(plots,nrow=2)
