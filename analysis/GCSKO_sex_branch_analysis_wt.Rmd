...
test vikash vs my dataframe
```{r}
## kasia's table for comparison
vikash <- read.delim("../bulk_rna-seq/enrichment/Kasia new cluster enrichment for Vikash.txt", sep = "\t", header = TRUE)
dim(vikash)
...
combined <- merge(screen_hits[which(screen_hits$`Gam phenotype screen` == "Both" | screen_hits$`Gam phenotype screen` == "Females" | screen_hits$`Gam phenotype screen` == "Males"), ], vikash[!vikash$cons.phenotype == "None", ], by.x = "new gene ID", by.y = "new.gene.ID", all.x = TRUE)
dim(combined)
```

set up df for python
```{r}
## read in modules
modules <- as.data.frame(gene_module_df_sex)
## take cols of interest
modules <- modules[ ,c("id", "module"), drop = FALSE]
## change the underscore to a dash
modules$id <- gsub("-", "_", modules$id)

## read in phenotypes from screen
screen_hits <- read_excel("../data/Screen/Modules_Clusters_Phenotypes.xlsx")
screen_hits_to_merge <- as.data.frame(screen_hits)
screen_hits_to_merge <- screen_hits_to_merge[ ,c("new gene ID", "Gam phenotype screen")]

## merge togerther to create final df
input_for_enrichment <- merge(modules, screen_hits_to_merge, by.x = "id", by.y = "new gene ID", all.x = TRUE)
names(input_for_enrichment) <- c("new gene ID", "Cluster", "cons phenotype")
## modify to match vikash table
## remove NA values
input_for_enrichment <- input_for_enrichment[!is.na(input_for_enrichment$`cons phenotype`), ]
## change the malenot enough power to none 
input_for_enrichment[input_for_enrichment$`cons phenotype` == "male not enough power", ]$`cons phenotype` <- "None"
##clean up
rm(screen_hits_to_merge)
```

do significance test in python - update this to R at a later date - Vikash wrote this for enrichment of screen hits in bulk data
```{python}
import os
import pandas as pd
...
def enrich_cluster_final():
    ...
    male_df=tmp[(tmp['cons phenotype']== 'Males')]
    female_df=tmp[(tmp['cons phenotype']== 'Females')]
    both_df=tmp[(tmp['cons phenotype']== 'Both')]


    ## enrichment analysis for male
    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_gametocyte_male.txt'
    #First argumnet: temperory dataframe
    #Second argument: list of male-specific genes
    #Third argumnet: file where we store enrichment analysis
    ...
    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_gametocyte_female.txt'
    apply_enrich(tmp ,female_df.index.to_list(),outfile)

    ## enrichment analysis for both sexes: : similar as male

    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_gametocyte_both.txt'
...
OLD LIST FROM KENT PAPER
```{r}
#DOZI_regulated_genes <- read.csv(file = "../data/Reference/DOZI_regulated_genes.csv", header = TRUE)
...
## if you want only discovered in both
#guerreiro_hits <- guerreiro_new[guerreiro_new$CITH_and_DOZI == 2, ]
## to get all genes with a pullldown
guerreiro_hits <- guerreiro_new[!guerreiro_new$CITH_and_DOZI == 0, ]

dozi_genes <- guerreiro_hits$new
## find out which modules they are in
df_dozi_gene_modules <- gene_module_df_sex[which(gene_module_df_sex$id %in% dozi_genes),]
print("dozi genes by module")
table(df_dozi_gene_modules$module)
...
## read in phenotypes from screen
screen_hits_to_merge <- as.data.frame(guerreiro_hits)
## create new col
screen_hits_to_merge$phenotype <- NA
screen_hits_to_merge[screen_hits_to_merge$CITH == 1, ]$phenotype <- "CITH"
screen_hits_to_merge[screen_hits_to_merge$DOZI== 1, ]$phenotype <- "DOZI"
screen_hits_to_merge[screen_hits_to_merge$CITH_and_DOZI == 2, ]$phenotype <- "CITH_and_DOZI"
screen_hits_to_merge <- screen_hits_to_merge[ ,c("new", "phenotype")]
## change the underscore to a dash
screen_hits_to_merge$new <- gsub("-", "_", screen_hits_to_merge$new)

## merge togerther to create final df
input_for_enrichment <- merge(modules, screen_hits_to_merge, by.x = "id", by.y = "new", all.x = TRUE)
names(input_for_enrichment) <- c("new gene ID", "Cluster", "cons phenotype")
## modify to match vikash table
## remove NA values
input_for_enrichment <- input_for_enrichment[!is.na(input_for_enrichment$`cons phenotype`), ]
...
def enrich_cluster_final():
    ...
    ## seprate the dataframes according sex-specific phenotypes
    male_df=tmp[(tmp['cons phenotype']== 'CITH')]
    female_df=tmp[(tmp['cons phenotype']== 'DOZI')]
    both_df=tmp[(tmp['cons phenotype']== 'CITH_and_DOZI')]


    ## enrichment analysis for male
    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_guerreiro_cith.txt'
    #First argumnet: temperory dataframe
    #Second argument: list of male-specific genes
    #Third argumnet: file where we store enrichment analysis
    apply_enrich(tmp,male_df.index.to_list(),outfile)
    ...
    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_guerreiro_dozi.txt'
    apply_enrich(tmp ,female_df.index.to_list(),outfile)

    ## enrichment analysis for both sexes: : similar as male

    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_guerreiro_both.txt'
    apply_enrich(tmp,both_df.index.to_list(),outfile)
if __name__ == '__main__':
...
Find out how many of the genes in each module has an asexual blood-stage phenotype gene
...
essential_genes <- essential_genes[, c("P..berghei.current.ID", "Phenotype" )]
names(essential_genes) <- c("id", "phenotype")
screen_hits_to_merge <- essential_genes

## merge togerther to create final df
input_for_enrichment <- merge(modules, screen_hits_to_merge, by.x = "id", by.y = "id", all.x = TRUE)
names(input_for_enrichment) <- c("new gene ID", "Cluster", "cons phenotype")
## modify to match vikash table
## remove NA values
input_for_enrichment <- input_for_enrichment[!is.na(input_for_enrichment$`cons phenotype`), ]
...
def enrich_cluster_final():
    ...
    ## seprate the dataframes according sex-specific phenotypes
    male_df=tmp[(tmp['cons phenotype']== 'Essential')]
    female_df=tmp[(tmp['cons phenotype']== 'Dispensable')]
    both_df=tmp[(tmp['cons phenotype']== 'Slow')]


    ## enrichment analysis for male
    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_bushell_essential.txt'
    #First argumnet: temperory dataframe
    #Second argument: list of male-specific genes
    #Third argumnet: file where we store enrichment analysis
    apply_enrich(tmp,male_df.index.to_list(),outfile)
    ...
    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_bushell_dispensable.txt'
    apply_enrich(tmp ,female_df.index.to_list(),outfile)

    ## enrichment analysis for both sexes: : similar as male

    outfile=current_folder+'/../data_to_export/module_enrichment_results/Module_enrichment_bushell_slow.txt'
    apply_enrich(tmp,both_df.index.to_list(),outfile)
if __name__ == '__main__':
...
## enrichment analysis in R first try
didn't work 
```{r}
## this was derived from an answer here: https://stackoverflow.com/questions/64969842/how-to-calculate-hypergeometric-test-for-each-row-in-data-frame-with-bonferroni
## the test dataset 
# Total_mRNAs = Total_genes_in_screen = N (constant)
# subset_mRNAs = Total_hits_in_screen = n (constant)
# Total_targets_targets = Total_genes_in_module = K
# subset_targets = Hits_in_module = k

# Total_genes_in_screen <- nrow(screen_hits[!is.na(screen_hits$`Gam phenotype screen`), ])
# ## this is if you take the screened genes in the cluster
# #Total_genes_in_screen <- as.data.frame(pc_screened$screened)
# Total_hits_in_screen <- nrow(screen_hits[which(screen_hits$`Gam phenotype screen` == "Both" | screen_hits$`Gam phenotype screen` == "Females" | screen_hits$`Gam phenotype screen` == "Males"), ] )
# Hits_in_module <- as.data.frame(table(df_hits_gene_modules$module))$Freq
# Total_genes_in_module <- as.data.frame(table(gene_module_df_sex$module))$Freq
# Module_names <- as.data.frame(table(gene_module_df_sex$module))$Var1
# 
# df1 <- data.frame(Module_names = Module_names, Total_genes_in_module = Total_genes_in_module, Hits_in_module = Hits_in_module, Total_hits_in_screen = rep(Total_hits_in_screen, length(Module_names)), Total_genes_in_screen = rep(Total_genes_in_screen, length(Module_names)))
# 
# library(broom)
# result = lapply(1:nrow(df1),function(i){
#        not_target_subset = df1$Total_genes_in_module[i] - df1$Hits_in_module[i]
#        not_subset = df1$Total_genes_in_screen[i] - df1$Total_hits_in_screen[i] - not_target_subset
#        
#        
#        M = cbind(c(df1$Hits_in_module[i],df1$Total_hits_in_screen[i]-df1$Hits_in_module[i]),
#              c(not_target_subset,not_subset))
#       
#        res = data.frame(Module_names=df1$Module_names[i],
...
# ## Adjust p value using bonferoni multiple hypothesis correction
