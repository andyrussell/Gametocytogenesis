...
ggplot(narrower, aes(x = value, color = key)) +
  geom_density(alpha = 0.5)

ggplot(merge, aes(x = log10(sqrt(left_arm_length^2 + right_arm_length^2)), y = proportion)) +
  geom_point() +
  geom_smooth(method = "lm")
ggplot(merge, aes(x = log10(sqrt(left_arm_length^2 + right_arm_length^2)), y = normd6toinputA)) +
  geom_point() +
  geom_smooth(method = "lm")

ggplot(merge, aes(x = Relative.Growth.Rate, y = proportion)) +
  geom_point() +
  geom_smooth(method = "lm")
ggplot(merge, aes(x = Relative.Growth.Rate, y = normd6toinputA)) +
  geom_point() +
  geom_smooth(method = "lm")
```
```{r}
...
new <- merge %>%
  ...
  filter(row_number() > 600 & row_number() < 700)
...
ggplot(comb, aes(x = Tax, y = texter, fill = n)) +
  geom_tile() +
  scale_fill_viridis()
```
