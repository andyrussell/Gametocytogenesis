...
noncontrol<-variance %>% filter(Condition=="GFPsort")
control<-variance %>% filter(Condition=="RFPsort")
...
ggplot(comparisonmerge,aes(label=substr(gene,8,20),y=log(0.5-abs(0.5-p)+0.000000000000000000001),x=difference,color=gene%in%candidates))+geom_point()+geom_text(size=4,color="black",nudge_y=1,angle=45)
...
shinyServer(function(input, output) {
  ...
  output$basicplot = renderPlot({
    ...
    ggplot(ungroup(filter(variance,gene==g,Condition%in%c("control","GFPsort","RFPsort"))),aes(x=Condition,color=Mouse,y=2^(meanlogprop-0.5),group=Mouse))+geom_point(position=position_dodge(width=0.2))+geom_line()+ggtitle(g)+scale_y_log10(breaks=number_ticks(4))+labs(y="Proportion",x="Stage")+geom_errorbar(aes(ymin=2^(meanlogprop-2*monotonicmedsd-.5),ymax=2^(meanlogprop+2*monotonicmedsd-.5)),width=0.2,position="dodge",alpha=0.5)+facet_wrap(~Pool, scales="free")
  },res=120)
  output$topplot = renderPlot({
    
    p= ggplot(thetable(),aes(x=GFPsort,y=RFPsort,color=phenotype))+geom_point()
    if(input$highlight){
    p= p+geom_point(data=filter(thetable(),gene%in% candidates),shape=1,size=3,color="black")+scale_color_manual(values = phenolevelscolor)
    }
    p= p+geom_point(data=filter(thetable(),gene == trimws(input$gene) | current_version_ID == trimws(input$gene)),shape=1,size=8,color="red",stroke=2)
    p
  },res=80)
