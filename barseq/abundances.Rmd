...
ggplot(merge,aes(x=log10(sqrt(left_arm_length^2+right_arm_length^2)),y=proportion))+geom_point()+geom_smooth(method="lm")
...
asexual<-read_csv("/Users/theo/gametocytes/gametocyteBarseq/otherdata/asexualscreendata.csv",col_names = T)
merge<-inner_join(oneversion,asexual)
orthologs<-read_tsv("/Users/theo/Dropbox/Sanger/gametocytes/gametocyteBarseq/otherdata/orthologs.txt",col_names = F)
orthomcl<-read_tsv("/Users/theo/Dropbox/Sanger/gametocytes/gametocyteBarseq/otherdata/allorthomcl.txt",col_names = T)
...
new<-merge %>% arrange(minmax) %>% mutate(texter=paste(gene,class)) %>% filter(row_number()>600 &row_number()<700  )
...
ggplot(comb,aes(x=Tax,y=texter,fill=n))+geom_tile()+scale_fill_viridis()
```
