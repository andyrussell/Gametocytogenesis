...
```
##Demonstration that superpools are useful

It's nice to have some sort of demonstration that our superpooling strategy has had an effect. The simplest way to do this might be to display the proportions in our superpool as compared to hypothetical proportions from mixing together the pools in the asexual screen. This turns out not to show much of an effect, possibly because the superpools have been in culture longer I think. But another way to get at this is to show that we have removed/reduced biases due to integration efficiency and fitness. That's what we do below.
```{r superpool}
proportions<- narrow %>% filter(Pool=="PbSTM101",Condition=="control") %>% group_by(gene) %>% summarise(proportion=mean(proportion))
olddisp<- asexual_screen_data #%>% filter(phenotype=="Dispensable")
merge<-inner_join(proportions,olddisp,by="gene")


ggplot(merge,aes(x=proportion,y=normd6toinputA))+geom_point()+scale_x_log10()+scale_y_log10()

merge$normd6toinputA=log10(merge$normd8toinputA)
merge$proportion=log10(merge$proportion)
merge$normd6toinputA=merge$normd6toinputA-mean(merge$normd6toinputA,na.rm=T)
merge$proportion=merge$proportion-mean(merge$proportion)
narrower<-merge %>% dplyr::select(gene, proportion,normd6toinputA) %>% gather("key","value",-gene)
#ggplot(narrower,aes(x=value,color=key))+geom_density(alpha=0.5)

#gplot(merge,aes(x=log10(sqrt(left_arm_length^2+right_arm_length^2)),y=proportion))+geom_point()+geom_smooth(method="lm")
#ggplot(merge,aes(x=log10(sqrt(left_arm_length^2+right_arm_length^2)),y=normd6toinputA))+geom_point()+geom_smooth(method="lm")

#ggplot(merge,aes(x=Relative.Growth.Rate,y=proportion))+geom_point()+geom_smooth(method="lm")
#ggplot(merge,aes(x=Relative.Growth.Rate,y=normd6toinputA))+geom_point()+geom_smooth(method="lm")


```
```{r}
proportions<- narrow %>% filter(Pool=="SP3",Condition=="control") %>% group_by(gene) %>% summarise(proportion=mean(proportion))
olddisp<- asexual_screen_data %>% filter(phenotype %in% c("Slow","Fast","Dispensable"))
merge<-inner_join(proportions,olddisp,by="gene")
...
merge$proportion=merge$proportion-mean(merge$proportion)
narrower<-merge %>% dplyr::select(gene, proportion,normd6toinputA) %>% gather("key","value",-gene)
ggplot(narrower,aes(x=value,fill=key))+geom_density(alpha=0.5)


a<-ggplot(merge,aes(x=Relative.Growth.Rate,y=proportion))+geom_point(alpha=0.2)+geom_smooth(method="lm")+labs(y="Relative proportion (powers\n of 10) in superpool",x="Relative growth rate")
b<-ggplot(merge,aes(x=Relative.Growth.Rate,y=normd6toinputA))+geom_point(alpha=0.2)+geom_smooth(method="lm")+labs(y="Relative proportion (powers\n of 10) in asexual screen",x="Relative growth rate")
c<-ggplot(merge,aes(x=sqrt(left_arm_length^2+right_arm_length^2),y=proportion))+geom_point(alpha=0.2)+geom_smooth(method="lm")+scale_x_log10()+labs(y="Relative proportion (powers\n of 10) in superpool",x="Geometric mean homology arm length")
d<-ggplot(merge,aes(x=sqrt(left_arm_length^2+right_arm_length^2),y=normd6toinputA))+geom_point(alpha=0.2)+geom_smooth(method="lm")+scale_x_log10()+labs(y="Relative proportion (powers\n of 10) in asexual screen",x="Geometric mean homology arm length")
library(patchwork)
...
ggplot(variance,aes(x=val1,y=val2,color=Condition))+ geom_rect(data=cors,xmin= -50,xmax=50,ymin= -50,ymax=50,aes(fill=-cor,x=NA,y=NA,group=paste(Pool,Condition,Mouse)))+
...
```{r}
narrow %>% filter(Condition %in% c("control","Negsort","GFPsort","RFPsort")) %>% group_by(gene) %>% summarise(min=min(proportion),count=n()) %>% arrange(-count,-min)
...
controlsmerge<- controlsset %>% group_by(Condition,Pool) %>% summarise(difference=gaussianMeanAndVarianceSplit(difference,differencesd^2,"mean"),differencesd=sqrt(gaussianMeanAndVarianceSplit(difference,differencesd^2,"variance")))
...
comparisonmerge<- comparisonnormalised %>% group_by(Condition,gene) %>% summarise(difference=gaussianMeanAndVarianceSplit(difference,differencesd^2,"mean"),differencesd=sqrt(gaussianMeanAndVarianceSplit(difference,differencesd^2,"variance")))
...
sextranscriptomesaverage <- sextranscriptomes %>% group_by(type,GeneID)%>% summarise(proportion=mean(proportion)) %>% spread(key=type,value=proportion) %>% mutate(malratio=mal/asexua,femalratio=femal/asexua)
...
ggplot(filter(gathered,class != "gainOfFemales"& class !="gainOfMales"),aes(x=stage,y=expression)) + 
      stat_summary(aes(y = expression,group = 1), fun.y=mean, colour="red", geom="line", size = 1)+facet_wrap(~class)+ expand_limits(y = 0)
```
...
tallied<-m2 %>% group_by(interpro,class) %>% summarise(n=n()) %>% spread(class,n, fill=0) 
totals<-m2 %>% count(class)


tallied<- tallied %>% ungroup %>% mutate( p1= phyper(lossOfMales-1,filter(totals,class=="lossOfMales")$n,filter(totals,class!="lossOfMales")$n,lossOfBoth+gainOfMales+lossOfFemales,lower.tail=F),
...
ggplot(abc,aes(x=diffmax.rfp.x,y=diffmax.rfp.y))+geom_point()

ggplot(abc,aes(x=diffmax.gfp.x,y=diffmax.gfp.y))+geom_point()
```
```{r, eval=FALSE}
...
pbmc <- CreateSeuratObject(raw.data = all_merge, min.cells = 3, min.genes = 200, 
    project = "10X_PBMC")
...
projected = ProjectPCA(pbmc, do.print = TRUE, pcs.print = 1:5, pcs.store = 17,
...
ggplot(arrange(newmat,PF3D7_0315600),aes(x=tsne1,y=tsne2,color=log(PF3D7_0315600)))+geom_point(size=1,alpha=0.5)+scale_color_viridis()
...
print("hi")
new<- merge %>% arrange(minmax) %>% filter(row_number()<80) %>% filter(PfID != "#N/A") %>% filter(phenotype != "Essential") %>%  filter(phenotype != "Slow")
#Use purr
library(purrr)
library(rlang)
...
makePlot<-function(x,y){
  ...
  p=ggplot(arrange(newmat,!! sym(x)),aes(x=tsne1,y=tsne2,color=log(!! sym(x))))+geom_point(size=1,alpha=0.5)+scale_color_viridis()+ggtitle(y)+guides(color=FALSE)
  return(p)
}
...
tally<-orthomcl %>% group_by(Tax,Group) %>% summarise(n=n()) %>% filter(Tax %in% taxa)
tally$Tax=factor(as.character(tally$Tax),levels=taxa)
new<-merge %>% arrange(minmax) %>% mutate(texter=paste(gene,class)) %>% filter(row_number()<70  )
...
ggplot(filter(comb,gene %in% analysedindetail),aes(x=Tax,y=texter,fill=n))+geom_tile()+scale_fill_viridis()
```
```{r paintercomparison}
asexual<-read_csv("otherdata/asexualscreendata.csv",col_names = T)
merge<-inner_join(oneversion,asexual)
new<-merge %>% arrange(minmax) %>% mutate(texter=paste(gene,class)) %>% filter(row_number()<200  )
painters3<-read_csv("otherdata/paintergams3.csv")
both<-inner_join(new,painters3,by=c("PfID"="GeneID"))
p1<-ggplot(both,aes(label=texter,y=as.numeric(`3D7 Gam Specific Stabilization`),color=class,x=fct_reorder(texter,minmax)))+geom_point()
library(plotly)
ggplotly(p1)
p2<-ggplot(both,aes(label=texter,y=as.numeric(`3D7 Gam Specific Transcription`),color=class,x=fct_reorder(texter,minmax)))+geom_point()
library(plotly)
ggplotly(p2)
