...
```{r superpool}
proportions <- narrow %>%
  filter(Pool == "PbSTM101", Condition == "control") %>%
  group_by(gene) %>%
...
ggplot(filt, aes(y = -n, x = time2, fill = class)) +
  geom_tile() +
  geom_line(data = filter(filt, !is.na(class), class != "No change"), aes(group = id, color = class, alpha = 0.5)) +
  labs(y = "Genes ranked at each timepoint by log2-fold change", x = "Timepoint (hours)") +
...
filt <- merge2 %>%
  group_by(timepoint) %>%
  arrange(pval) %>%
  filter(row_number() < 2500) %>%
  mutate(n = row_number())
library(viridis)
ggplot(filt, aes(y = -n, x = time2, fill = minmax)) +
  geom_tile() +
  geom_line(data = filter(filt, !is.na(class), class != "No change"), aes(group = id, color = class, alpha = 0.5)) +
